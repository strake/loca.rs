...
unsafe impl Alloc for Heap {
    ...
    #[inline]
    unsafe fn grow_in_place(&mut self, ptr: *mut u8, layout: Layout, new_layout: Layout)
                            -> Result<(), CannotReallocInPlace> {
        debug_assert!(new_layout.size() >= layout.size());
        debug_assert!(new_layout.align() == layout.align());
        if 0 == __rust_grow_in_place(ptr,
                                     layout.size(),
                                     layout.align(),
                                     new_layout.size(),
                                     new_layout.align()) { Err(CannotReallocInPlace) }
        else { Ok(()) }
    }
    ...
    unsafe fn shrink_in_place(&mut self,
        ...
        debug_assert!(new_layout.size() <= layout.size());
        debug_assert!(new_layout.align() == layout.align());
        if 0 == __rust_shrink_in_place(ptr,
                                       layout.size(),
                                       layout.align(),
                                       new_layout.size(),
                                       new_layout.align()) { Err(CannotReallocInPlace) }
...
mod tests {
    ...
    #[bench]
    fn alloc_owned_small(b: &mut Bencher) { b.iter(|| { let _ = Box::new(1).unwrap(); }) }
}
